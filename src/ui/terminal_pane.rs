...
impl TerminalPane {
    ...
    pub(super) fn presentation_succeeded(
        ...
        let graphics_presented = self.graphics_cache.update(cx, |cache, cx| {
            cache.mark_presented(graphics_attempt, &screen.graphics, cx)
        });
    ...
    fn apply_backing_scale(
        ...
        self.last_geometry = None;
        if force_resources
            || self.render_lifecycle.update_scale(factor) == ScaleChange::ScaleResources
        {
...
impl Render for TerminalPane {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let graphics_snapshot = display_screen.graphics.clone();
        let renderer_recovery_pending = self
            .pending_recovery
            .is_some_and(|pending| pending.action == RecoveryAction::RendererResources);
        let graphics_attempt_allowed =
            !renderer_recovery_pending || self.recovery_retry_requested == self.pending_recovery;
        ...
        let (graphics, graphics_attempt, graphics_attempted) =
            self.graphics_cache.update(cx, |cache, cx| {
                ...
                match cache.sync(&graphics_snapshot, window, cx) {
        ...
        let preedit = Arc::ptr_eq(&display_screen, &self.screen)
            .then(|| self.preedit_layout())
            .flatten();
        ...
        let terminal_grid = TerminalGridElement::new(
            ...
            TerminalGridConfiguration {
                ...
                fallback: (!Arc::ptr_eq(&display_screen, &self.last_valid_screen))
                    .then(|| (Arc::clone(&self.last_valid_screen), fallback_graphics)),
                paint_fault: {
                    #[cfg(test)]
                    {
...
mod tests {
        ...
        for generation in 2..18 {
            ...
            let result = cx.update(|window, cx| {
                cache.update(cx, |cache, cx| {
                    ...
                    cache.sync(&screen.graphics, window, cx)
                })
            });
    ...
    fn stale_graphics_attempt_cannot_roll_back_a_newer_same_generation_stage(
        ...
        let attempt_a = cx.update(|window, cx| {
            cache
                .update(cx, |cache, cx| cache.sync(&screen_a.graphics, window, cx))
                .unwrap()
                .token
        });
        let attempt_b = cx.update(|window, cx| {
            cache
                .update(cx, |cache, cx| cache.sync(&screen_b.graphics, window, cx))
                .unwrap()
                .token
        });
        ...
        cache.update(cx, |cache, cx| {
            ...
            assert!(cache.mark_presented(attempt_b, &screen_b.graphics, cx));
        });
        assert_eq!(
            cache.read_with(cx, |cache, _| cache.cached_image_keys()),
            vec![crate::terminal::ImageKey {
