...
| **SSH Runtime Host Facts** | The validated OpenSSH executable, trusted runtime fallback root, and local IPC address-length constraint selected once by Host Composition without defining platform operations. | SSH Platform Adapter, global host defaults, shared hardcoded paths |
...
- Portable application-path and managed-host policy owns XDG precedence, runtime-owner namespaces, unpredictable temporary allocation, artifact registration, mutation sequencing, conflicts, stale-owner refusal, and cleanup decisions. Secure publication retains the prepared identity through commit and rolls back post-publication validation failures; exact removal uses private quarantine and identity revalidation. A **Secure Filesystem Adapter** exposes no raw descriptor, inode, ownership field, mode bit, or native error.
