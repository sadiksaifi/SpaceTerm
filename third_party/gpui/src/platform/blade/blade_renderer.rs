...
pub struct BladeRenderer {
    ...
    scene_texture: gpu::Texture,
    scene_texture_view: gpu::TextureView,
...
impl BladeRenderer {
    pub fn new<I: raw_window_handle::HasWindowHandle + raw_window_handle::HasDisplayHandle>(
        ...
        let (path_intermediate_msaa_texture, path_intermediate_msaa_texture_view) =
            ...
            .unzip();
        let (scene_texture, scene_texture_view) = create_scene_texture(
            &context.gpu,
            surface.info().format,
            config.size.width,
            config.size.height,
    ...
    fn update_drawable_size_impl(&mut self, size: Size<DevicePixels>, always_resize: bool) {
        ...
        if always_resize || gpu_size != self.surface_config.size {
            ...
            self.gpu.destroy_texture_view(self.scene_texture_view);
            self.gpu.destroy_texture(self.scene_texture);
            (self.scene_texture, self.scene_texture_view) = create_scene_texture(
                &self.gpu,
                self.surface.info().format,
                gpu_size.width,
                gpu_size.height,
    ...
    fn ensure_backdrop_textures(&mut self) {
        ...
        if self
            .backdrop_textures
            .as_ref()
            .is_some_and(|textures| textures.size == size)
        {
            return;
        }
        ...
        let quarter_size = gpu::Extent {
            width: size.width.div_ceil(4).max(1),
            height: size.height.div_ceil(4).max(1),
            depth: 1,
        };
    ...
    fn draw_backdrop_filter(
        ...
        for pass_index in first_pass..3 {
            ...
            let Some(scissor) = backdrop_scissor(output, halo, target_size, size) else {
                continue;
            };
            let sigma = match pass_index {
                0 => filter.radius.0 * target_size.width as f32 / size.width as f32,
                1 => filter.radius.0 * target_size.height as f32 / size.height as f32,
                _ => 0.0,
            };
    ...
    pub fn draw(&mut self, scene: &Scene) {
        ...
        self.command_encoder.init_texture(self.scene_texture);
        ...
        if let mut pass = self.command_encoder.render(
            ...
            encoder.bind(
                0,
                &ShaderPresentData {
                    t_scene: self.scene_texture_view,
                },
            );
            encoder.draw(0, 4, 0, 1);
        }
