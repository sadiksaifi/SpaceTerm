...
| **Local Filesystem Authority** | The platform-neutral owner of local directory validation, listing, creation, exact spelling, opaque retained identity, canonical file resolution, emission metadata, and revalidation policy shared by Local Workspaces, Terminal Hyperlinks, and File Preview. | Local Workspace Adapter, broad Filesystem Platform Interface |
| **Workspace Directory Identity** | An opaque retained identity for a validated local directory, comparable without knowledge of its native representation and unable to authorize a replacement object. | device/inode pair, path spelling, remote Physical Directory Identity |
...
| **macOS Keybinding Profile** | The explicitly selected mapping of semantic Actions and reusable Text Input, Modal, and Command Palette control profiles to the shipped macOS shortcuts; it is immutable and not user-configurable. | semantic Actions, automatic platform detection |
...
| **Active Tab** | The one Tab selected within a Workspace. | selected Tab, focused Tab |
| **Focused Pane** | The one Pane selected by a Tab for Pane operations and focus restoration. | Active Pane, selected Pane, Terminal Input Focus |
| **Terminal Input Focus** | Transient truth that a Pane may accept terminal key input because its Workspace and Tab are Active, it is the Focused Pane and current responder, its Operating-System Window and SpaceTerm are active, and no temporary UI owner blocks it. | Focused Pane, selected terminal |
...
| **System Directory Selection** | The application-owned GPUI directory chooser opened only from the Workspace Picker footer; it returns to the unchanged picker on cancellation and uses the same validation path on selection. | primary project picker, canonical selection path |
| **Zoomed Pane** | The Focused Pane temporarily shown alone while its Tab's Pane Layout remains intact. | Maximized Tab, fullscreen Pane |
...
| **SSH Runtime Host Facts** | The validated OpenSSH executable, trusted runtime fallback root when the captured environment needs one, and local IPC address-length constraint selected once by Host Composition without defining platform operations. | SSH Platform Adapter, global host defaults, shared hardcoded paths |
| **SSH Process Adapter** | The narrow host-selected Interface for process creation, nonblocking observation, termination mechanics, and final reap, while portable supervision owns all lifecycle policy. | SSH process owner, raw child process, broad Unix Adapter |
| **Secure Filesystem Adapter** | The narrow host-selected Interface for race-resistant application-path and managed-host mechanics whose policy, ordering, ownership, and cleanup decisions remain portable. | filesystem policy, raw descriptor wrapper, broad SSH storage Adapter |
| **AskPass Broker** | The portable owner of the bounded AskPass protocol, capability verification, one active Authentication Prompt, cancellation, response zeroization, observation, and teardown ordering. | native prompt, socket server, authentication secret cache |
| **AskPass Local IPC Adapter** | The narrow host-selected listener and stream mechanism for one AskPass endpoint, including exact endpoint cleanup and independent same-user peer authentication before frame parsing. | AskPass Broker, SSH Platform Adapter, unauthenticated socket |
...
| **Authenticated Runtime Observation Owner** | The optional application-scoped portable owner selected by Host Composition before startup capture, with one-shot Pane and Terminal Session claiming, bounded protocol and writer ownership, and final completion authority. | process-global observer, Acceptance Platform Adapter |
| **Observation Lease** | Non-cloneable authority binding one claimed production Pane and its separately consumed Terminal Session lease to the exact Authenticated Runtime Observation Owner. Removal or completion cannot transfer that authority to a successor. | global pending session, reusable observation token |
...
| **Native PTY Adapter Factory** | The narrow application-selected construction Interface that accepts one complete prepared Shell Launch Plan and platform-neutral geometry and returns the Adapter and termination authority transferred into its Native PTY Owner. | platform singleton, broad Platform trait, PTY owner |
...
| **Shell Launch Plan** | The platform-neutral Module that prepares the complete local Shell Process or remote Terminal Session Channel executable, arguments, working directory, environment handling, integration, and identities before Native PTY Adapter construction. Its prepared value is owned, non-cloneable, and redacted in diagnostics. | PTY command builder, macOS shell policy |
| **Local Path Semantics** | Explicit portable policy for local absolute paths, home-relative spelling, directory display, completion boundaries, and file-URL conversion, selected by Host Composition and retained by Local Filesystem Authority and Local Terminal Metadata. | ambient host detection, slash-only UI parsing |
| **Shell Insertion Dialect** | The explicitly selected local shell quoting policy used to encode File Insertion before Paste Payload validation; the current composition selects POSIX quoting. | Operating-System quoting, remote shell command |
| **File Preview** | Pane-owned local-file preview eligibility, identity revalidation, replacement, failure cleanup, and teardown through an independent presentation capability. The macOS Adapter uses Quick Look and its Desktop Profile retains that label. | Quick Look policy, broad Platform Interface |
...
| **Terminal Attention** | Pane-owned unread and visual state derived from bounded BEL or command-completion events without moving Terminal Input Focus, with portable application-wide coordination of shared native effects. | global alarm, focus request |
| **Application Activity** | The application-active fact supplied independently of GPUI key-window ownership and combined with product and responder facts by portable lifecycle policy. | main window existence, Focused Pane |
| **Window Visibility Source** | An injected capability that observes minimized, occluded, and live-resize facts for one exact Operating-System Window and owns only its native observation lifetime. | global active window poller, Render Lifecycle |
| **Wheel Phase Enrichment** | The narrow injected capability supplying only cancellation and momentum distinctions unavailable in GPUI's ordinary gesture phases. | native wheel policy, scroll routing |
...
| **Terminal Hyperlink** | A validated immutable target mapped to complete Terminal Presentation cells, presented as active only during platform-modified pointer hover, and opened only by a platform-modified same-generation activation. | arbitrary terminal text, automatic file execution |
...
| **Secure Event Input** | Application-wide keyboard protection coordinated by portable Rust and held only while the application is active and exactly one live registered Pane both reads hidden canonical PTY input and owns Terminal Input Focus; Carbon supplies current macOS transitions. | password text, per-Pane secure mode |
...
| **File Insertion** | Ordered local absolute paths from validated clipboard file URLs or GPUI drag/drop, converted within input and aggregate bounds according to the explicitly selected local Shell Insertion Dialect before becoming a Paste Payload. | raw URL paste, direct PTY write |
| **Native Terminal Services** | The platform-neutral Module owning Selection publication, clipboard and File Insertion intake, Paste Payload and OSC 52 policy, contextual-action and Terminal Hyperlink revalidation, File Preview eligibility and lifecycle, and exact Services request authority. | grab-bag platform Adapter, independent input path |
| **Native Terminal Service** | One request governed by Native Terminal Services, without direct Terminal Emulator mutation or PTY input. | direct PTY service, native policy |
| **Services Request** | A one-shot operation bound to one Operating-System Window endpoint and exact Workspace, Tab, Pane, Terminal Session, focus epoch, and hierarchy generation, with synchronous send and origin-bound return authority. | current-pane request, reusable requestor |
...
| **Terminal Accessibility Model** | A platform-neutral, Pane-owned editable text-area projection whose cell-atomic UTF-16 ranges map visible and retained terminal text, selected terminal font metadata, Selection, Cursor, and bounds back to logical terminal cells. | flattened cell string, screen-reader transcript |
| **Terminal Accessibility Adapter** | The narrow platform-neutral Interface owned by one Pane to publish its Terminal Accessibility Model, geometry, font request, hierarchy order, visibility, focus, and coalesced notification facts; application composition injects its factory through Workspace, Tab, and Pane construction. | Pane AppKit element, broad Platform trait |
| **macOS Terminal Accessibility Adapter** | The current production Terminal Accessibility Adapter that privately owns AppKit objects, roles and attributes, coordinate conversion, hierarchy registration, native notifications, and teardown. | platform-neutral accessibility model, Linux accessibility implementation |
| **Accessibility Selection Authority** | A platform-neutral Terminal Session command sender supplied to a Terminal Accessibility Adapter for generation-bound, complete-grapheme Selection requests; the worker alone validates and applies them and a stopped worker accepts none. | native emulator mutation, macOS-only Selection callback |
| **Render Lifecycle** | Portable Pane-owned visibility, animation, scale, and newest-generation presentation state that consumes GPUI, product, and exact-window facts and schedules frames only when its surface can present them. | render loop, global animation timer |
...
| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Create Scratch Workspace** | Add and activate a new Scratch Workspace with its initial Tab and Pane. | Create Workspace, start session, attach session |
...
| **Open Local Project** | Confirm one readable local directory through the Workspace Picker, or its explicit System Directory Selection, and create or activate its Local Project Workspace. | Open repository, convert Workspace |
...
| **Zoom Pane** | Temporarily present the Focused Pane alone without changing the Pane Layout. | Maximize Tab, fullscreen |
| **Restore Panes** | End Pane zoom and present the Tab's full Pane Layout again. | Unmaximize Tab, unzoom Tab |
...
- A **Local Project Workspace** preserves its exact selected or typed **Project Root** spelling and deduplicates equivalent selections by opaque retained **Workspace Directory Identity**.
- **Host Composition** selects one **Local Filesystem Authority** and passes it through Workspace, picker, child factory, and Terminal Session construction. The domain compares and retains identities without native fields; test identities use explicit logical labels.
- **Local Filesystem Authority** validates the exact directory spelling, including symlinks, and observes its identity around readability checks. Child creation and Local Project activation reject removal, replacement, retargeting, unreadability, and identity mismatch without granting a successor inherited authority. Failures and Debug values are content-free.
- A local-file **Terminal Hyperlink** retains its input spelling, canonical presentation path, and exact file lease. Activation and every File Preview presentation revalidate both paths through the same authority. The current narrow macOS identity Adapter only captures nonblocking object leases without reading contents; portable Rust owns policy and closed error mapping.
- Local-file emission metadata is a versioned opaque token with no path or native identity fields. One **Terminal Emulator** retains at most 32 emitted leases and releases them on teardown; unknown, foreign, malformed, oversized, or evicted tokens are inert. The application-wide authority reserves capacity before opening and bounds all Local File leases, including retained snapshots, to 64 and all identity handles to 96. Exhaustion refuses new authority until leases are released. A retained typed target cannot authorize a replacement object.
...
- The **System Directory Selection** remains behind the open **Workspace Picker**; cancellation restores the picker unchanged, while selection joins the same background validation and Local Project identity-deduplication flow.
- Only the **Directory Authority** may update a Scratch **Workspace Directory**. Closing it promotes the first remaining Pane in Pane Layout order, or the root Pane of the first remaining Tab when its Tab closes.
- A valid promoted Pane report is adopted immediately. A missing or invalid report retains the previous Workspace Directory.
- Create Tab and Split Pane revalidate the **Workspace Directory** before mutation. Unavailability blocks new children without stopping existing Terminal Sessions and clears after successful validation.
...
- **Host Composition** selects **SSH Runtime Host Facts**, one **SSH Process Adapter**, one **Secure Filesystem Adapter**, the Control Connection socket capability, and one **AskPass Local IPC Adapter**. These independent capabilities are constructor wiring and never form a broad SSH Platform Interface.
- Portable SSH policy consumes the selected OpenSSH executable without exposing its path, applies the OpenSSH 8.2 minimum-version rule, validates every command specification, and clears ambient process environment before applying the captured allowlist. Broker-authorized process launches add the exact AskPass overlay through zeroizing capability ownership; an authorized **Terminal Session Channel** receives no AskPass transport state and fails closed if its live Control Connection is unavailable.
- Portable application-path and managed-host policy owns XDG precedence, runtime-owner namespaces, unpredictable temporary allocation, artifact registration, mutation sequencing, conflicts, stale-owner refusal, and cleanup decisions. An absolute captured XDG runtime root needs no host fallback capture or canonicalization. Secure configuration reads and publication serialize through an identity-verified parent-directory lock; rollback restores a predecessor only after withdrawing and validating its own publication, preserving unowned successors. Exact removal uses private quarantine and identity revalidation. A **Secure Filesystem Adapter** exposes no raw descriptor, inode, ownership field, mode bit, or native error.
- Portable process supervision owns cancellation, deadlines, escalation, descendant-cleanup decisions, stale observation rejection, and exactly-once cleanup. Runtime, socket, and alias ownership remains retained until descendant termination, leader reap, and reader completion. Post-reap cleanup removes only a previously registered socket and never claims an unregistered replacement. An **SSH Process Adapter** exposes no process identifier, signal, process group, or raw native failure.
- The **AskPass Broker** authenticates the exact broker process before the helper sends a frame, validates the same-user helper before parsing its bounded request frame, compares the capability in constant time, presents at most one generation-bound **Authentication Prompt**, zeroizes every secret response, and releases its exact endpoint once during teardown.
- AskPass capability entropy is portable. The current macOS **AskPass Local IPC Adapter** privately retains Unix listener and stream mechanics plus same-user peer inspection; helper dispatch still precedes startup capture and bypasses GPUI.
- OpenSSH executable and runtime paths, local IPC locations, capability tokens, prompts, replies, authentication material, SSH Destinations, environment values, process identities, and raw native failures never enter Debug output, errors, **Local Diagnostics**, or terminal state.
...
- Application composition selects the concrete **Native PTY Adapter Factory** and passes it through **Terminal Session** construction. The **Native PTY Owner** supplies a complete prepared **Shell Launch Plan** and platform-neutral geometry values and owns only the returned Adapter parts.
- The **Shell Launch Plan** owns executable and argument selection, exact working-directory validation, **Shell Integration**, inherited-environment handling, **Terminal Capability Identity**, **Compatibility Identity**, and resource decisions. Application composition injects the selected shell, resources, captured integration environment and mode, compatibility eligibility, search-path separator, and fallback directories.
- Remote **Shell Launch Plan** preparation rechecks live channel authority and preserves prepared SSH environment or its existing fallback; it never grants local authority to a **Remote Workspace Directory**. Channel consumption is single-use even when later launch fails.
- The macOS **Native PTY Adapter Factory** consumes only prepared process values and owns PTY configuration, spawning, input/output acquisition, resize, hidden-input inspection, termination, waiting, and reaping mechanics. Launch descriptions and typed failures expose no secret values or sensitive paths.
...
- The **Native PTY Owner** Interface exposes only platform-neutral launch, geometry, input/output, lifecycle, and closed failure classifications. Construction failures never carry concrete paths, command values, platform mechanisms, or raw Operating-System errors. The macOS Adapter owns controlling-terminal and process-group mechanics; a Linux Adapter is intentional future work, and no Windows Adapter is defined.
...
- GPUI window activation supplies key **Operating-System Window** ownership. **Application Activity** is independent: a retained main window does not prove the application is active. Portable focus policy combines both with Active Workspace, Active Tab, Focused Pane, responder ownership, and temporary UI blockers.
...
- **Native Terminal Services** uses GPUI where it preserves behavior, portable Rust for policy and lifecycle, and independent capability-specific Adapters only for irreducible mechanics. Application composition injects those capabilities through **Workspace**, **Tab**, **Pane**, and **Terminal Session** construction.
- The remaining macOS mechanics are formatted NSPasteboard publication, file-URL discovery, synchronous worker clipboard transport, QLPreviewView/NSPanel presentation, and NSServices responder integration. URL decoding, request identity, reentrancy gates, preview revalidation, and teardown policy are Rust-owned.
- A **Services Request** retains its exact origin and Window endpoint. Stale, overlapping, reentrant, consumed, or owner-retired requests cannot target a successor; return authority is consumed before invoking callbacks.
- A completed **Selection** copy publishes plain text and HTML synchronously so an immediate paste observes it. A **Remote Pane** never reads the file clipboard, and invalid file intake never falls through to ordinary text.
- A **Native Terminal Service** may submit a **Paste Payload** only while its Pane owns **Terminal Input Focus**, and may offer File Preview only for an existing validated local-file **Terminal Hyperlink**.
- An active **Terminal Hyperlink** uses a pointing hand and destination preview, preserves every terminal-supplied underline, and adds one font-aligned underline only to the contiguous hovered occurrence, continuing across soft-wrapped rows; separate occurrences of the same target remain inactive, and modifier changes update this presentation without requiring pointer movement.
...
- A **Terminal Accessibility Model** treats each retained text-bearing cell as one complete accessibility grapheme, rejects string reads that would expose only part of that cell, normalizes Selection requests to complete intersected cells, observes immutable terminal state, and never mutates the Terminal Emulator directly.
- Application composition selects the **Terminal Accessibility Adapter** factory and passes it through **Workspace**, **Tab**, and **Pane** construction. Each Pane owns only the resulting Interface; the current **macOS Terminal Accessibility Adapter** retains native implementation details.
- **Accessibility Selection Authority** submits requests through the reliable **Terminal Session** command lane. Hierarchy removal synchronously retires native registration and Selection authority, Pane destruction releases its Adapter, and requests after worker shutdown are inert.
...
- Cursor blink, text blink, and visual effects require active application and key-window ownership, visible product state, and a surface that is neither minimized, occluded, nor live-resizing. Losing eligibility cancels Pane-owned animation work; stale task generations cannot resume it.
- A **Window Visibility Source** remains bound to the exact captured Operating-System Window across activation changes. Native notifications enqueue coalesced wakeups; portable Rust owns reduction and scheduling. Only authenticated **Runtime Observation** creates the bounded 50-millisecond polling monitor.
- GPUI supplies ordinary wheel start, movement, and end phases. **Wheel Phase Enrichment** supplies only missing cancellation and momentum facts; portable Rust preserves independent axis remainders and routes application mouse reporting before Alternate Screen arrows and Primary Screen Scrollback.
- Pane surface behavior uses GPUI first, portable Rust for policy and lifecycle, and independent capability-specific Adapters only for irreducible mechanics. Application composition injects these through every Workspace, Tab, Pane, replacement, split, Local, and Remote creation path. Constructor wiring defines no platform operations.
- **Secure Event Input** registrations belong to one application-scoped portable coordinator shared across Operating-System Windows. Zero or multiple eligible Panes refuse ownership. Focus loss, deactivation, Terminal Session completion, and Pane removal clear eligibility; permanent retirement rejects stale updates. Failed enable grants no ownership, and failed disable refuses logical ownership while retaining physical accounting for release retry.
- Pane teardown invalidates task generations and cancels owned work, retires portable attention and secure-input ownership, and releases Render Lifecycle state before releasing its native resources. Repeated close is inert; stale callbacks cannot act on a successor Pane.
- **Host Composition** consumes the acceptance launch capability before startup environment capture and injects one optional **Authenticated Runtime Observation Owner** through Workspace, Tab, and Pane construction. Absence creates no acceptance resources, channels, writers, tasks, or polling monitor.
- An **Observation Lease** is claimed once by one production Pane; its separately consumed Terminal Session lease travels explicitly through the Workspace-bound factory. There is no global handoff or successor authority. Pane and Terminal Session producer handles are revoked independently; final sealing closes admission and drains in-flight producers before the final sample.
- Portable observation policy owns bounded framing, field and sequence validation, Failure Action phases, writer cadence, drain, acknowledgement, content-free failures, and final sealing. GPUI quit transfers completion to background ownership; its post-event-loop fallback retains the same completion result.
- Remaining native observation mechanics are independent private-owner filesystem facts, exact packaged-executable and read-only mount authentication, private local byte transport, and sleep-inclusive clock conversion. Native package fields are consumed and emitted only by the packaged-executable capability on its authenticated launch transport; they are never runtime facts or diagnostics.
...
- **Terminal Metadata** belongs to exactly one **Terminal Session**; Pane and Tab chrome may consume its sanitized immutable values but never terminal control bytes or live emulator state.
...
- **Terminal Attention** remains scoped to its owning Pane and Tab; focus gain or accepted input clears it, while repeated native effects are rate-limited and notifications occur only when SpaceTerm is inactive.
- The portable Terminal Attention runtime shares Dock request ownership and five-second notification aggregation across Panes and Operating-System Windows. One-second Dock rate limiting, scheduling generations, fresh activity checks, and cancellation remain Rust-owned; removing one Pane cannot cancel another Pane's pending demand.
- Notification authorization, provisional-authorization coordination, delivery generations, and stale completion rejection are portable policy. Native sound, Dock operations, and notification-center mechanics use independent capability Interfaces; their failures expose only closed, content-free classifications.
...
- "Active," "focused," and "selected" were used as near-synonyms; reserve **Active** for Workspace and Tab state and **Focused** for Pane state.
