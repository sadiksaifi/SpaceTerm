...
- Portable application-path and managed-host policy owns XDG precedence, runtime-owner namespaces, unpredictable temporary allocation, artifact registration, mutation sequencing, conflicts, stale-owner refusal, and cleanup decisions. Exact removal uses private quarantine and identity revalidation. A **Secure Filesystem Adapter** exposes no raw descriptor, inode, ownership field, mode bit, or native error.
- Portable process supervision owns cancellation, deadlines, escalation, descendant-cleanup decisions, stale observation rejection, and exactly-once cleanup. Runtime, socket, and alias ownership remains retained until descendant termination, leader reap, and reader completion. An **SSH Process Adapter** exposes no process identifier, signal, process group, or raw native failure.
