...
- An active **Terminal Hyperlink** uses a pointing hand and destination preview, preserves every terminal-supplied underline, and adds one font-aligned underline only to linked cells that have no underline of their own; modifier changes update this presentation without requiring pointer movement.
