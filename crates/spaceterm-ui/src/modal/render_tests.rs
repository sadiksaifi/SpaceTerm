...
fn bounds_horizontally_contain(outer: Bounds<gpui::Pixels>, inner: Bounds<gpui::Pixels>) -> bool {
...
#[gpui::test]
fn critical_alert_renders_bounded_semantic_treatment_without_accessory(cx: &mut TestAppContext) {
...
fn alert_intent_presentations_use_distinct_markers_and_semantic_paint() {
    ...
    let informational = alert_intent_presentation(super::super::AlertIntent::Informational, paint);
    ...
    let window = open_action_geometry_window(
        ...
        vec![
            ModalAction::new(
                "save",
                concat!(
                    "変更内容を安全に保存して作業を続ける前に、",
                    "関連するすべての設定と未保存の情報を確認し、",
                    "現在のワークスペースに影響する変更内容をもう一度確認してから、",
                    "この操作を完了する",
                ),
                ModalActionRole::Affirmative,
                "cjk-save",
...
fn constrained_scaled_dialog_reaches_long_header_body_and_every_action_vertically(
    ...
    for _ in 0..40 {
            ...
            .zip(["geometry-dialog-body-start", "geometry-dialog-body-end"])
        {
            ...
            *bottom_reached |= content.bottom() > body.top() && content.bottom() <= body.bottom();
        }
        cx.simulate_event(ScrollWheelEvent {
...
fn constrained_scaled_alert_reaches_long_title_message_and_actions_vertically(
    ...
    for _ in 0..20 {
        ...
        title_bottom_reached |= title.bottom() > header.top() && title.bottom() <= header.bottom();
        cx.simulate_event(ScrollWheelEvent {
            position: header.center(),
            delta: ScrollDelta::Pixels(point(px(0.0), px(-12.0))),
            modifiers: Modifiers::default(),
...
fn scaled_metrics_keep_actions_wrapped_and_reachable(cx: &mut TestAppContext) {
    ...
    let footer = cx
        .debug_bounds("modal-footer-1")
        .expect("footer should render");
    let actions = ["modal-action-scaled-continue", "modal-action-scaled-cancel"].map(|selector| {
        cx.debug_bounds(selector)
...
#[gpui::test]
fn cancellable_progress_focuses_cancel_and_space_requests_cancellation(cx: &mut TestAppContext) {
    install_test_catalogs(cx);
    let outcome = Rc::new(RefCell::new(None));
    let root_outcome = outcome.clone();
    let (root, cx) = cx.add_window_view(move |_, cx| AlertFixture {
    ...
    assert!(
        requests.borrow().as_slice() == [ModalActivationSource::Space]
            ...
            && initial.0.is_some_and(|(_, progress, generation)| {
                progress.cancellation_capable && !progress.cancellation_enabled && generation == 0
            })
...
#[gpui::test]
fn modal_survives_when_the_caller_does_not_retain_its_dismissal_handle(cx: &mut TestAppContext) {
    install_test_catalogs(cx);
    let (root, cx) = cx.add_window_view(|_, cx| AlertFixture {
        invoker: cx.focus_handle().tab_stop(true),
        underlay_activations: Rc::new(Cell::new(0)),
...
fn stale_progress_handle_clone_cannot_overwrite_a_newer_update(cx: &mut TestAppContext) {
    ...
    let result = cx
        .update(|window, cx| stale.update(ProgressDialogUpdate::new().status("Older"), window, cx));
...
#[gpui::test]
fn programmatic_progress_completion_restores_ordinary_predecessor_focus(cx: &mut TestAppContext) {
    install_test_catalogs(cx);
    let (root, cx) = cx.add_window_view(|_, cx| AlertFixture {
        invoker: cx.focus_handle().tab_stop(true),
        underlay_activations: Rc::new(Cell::new(0)),
...
fn queued_progress_update_should_transfer_runtime_and_generation_on_promotion(
    ...
    assert!(
        stale_result
            ...
            && promoted.1.detail.as_ref().map(|detail| detail.as_ref()) == Some("Retained detail")
...
#[gpui::test]
fn queued_progress_complete_should_deliver_completed_and_only_closed_once(cx: &mut TestAppContext) {
    let (root, _, _, cx) = alert_window(cx);
    let outcomes = Rc::new(RefCell::new(Vec::new()));
    let lifecycle = Rc::new(RefCell::new(Vec::new()));
    let handle = cx.update(|window, cx| {
...
#[gpui::test]
fn caller_removal_should_resolve_exact_queued_progress_without_opening(cx: &mut TestAppContext) {
    ...
    fn present(&mut self, window: &Window, cx: &mut Context<Self>) {
        ...
        self.presentation = Some(
            Dialog::new(
                ...
                "Save Changes",
                vec![
                    ModalAction::new("save", "Save", ModalActionRole::Affirmative, "pending-save")
                        .default_action(true),
                    ModalAction::new(
                        "cancel",
                        "Cancel",
...
fn public_progress_replacement_rejects_cancellation_terminal_and_update_authority(
    ...
    let (cancel_result, completion_result, dismissal_result, update_result, current) =
        cx.update(|window, cx| {
            (
...
fn dialog_programmatic_completion_disarms_public_body_button_before_release(
    ...
    let (root, activations, operation, controls_are_idle, _, cx) = dialog_body_button_window(cx);
    let button = cx
...
#[gpui::test]
fn dialog_active_replacement_disarms_public_body_button_before_release(cx: &mut TestAppContext) {
...
#[gpui::test]
fn outside_release_disarms_modal_action_before_blocking_underlay_input(cx: &mut TestAppContext) {
    let (_, underlay, outcome, cx) = alert_window(cx);
    let cancel = cx
        .debug_bounds("modal-action-cancel")
        .expect("cancel action should render");
...
#[gpui::test]
fn dialog_programmatic_completion_rejects_another_operating_system_window(cx: &mut TestAppContext) {
    let (root, cx) = dialog_completion_focus_window(cx);
    cx.update(|window, cx| root.update(cx, |root, cx| root.present(window, cx)));
    cx.run_until_parked();
    let completion = root
...
#[gpui::test]
fn dialog_programmatic_completion_does_not_steal_from_a_newer_focus_owner(cx: &mut TestAppContext) {
    let (root, cx) = dialog_completion_focus_window(cx);
...
#[gpui::test]
fn dialog_initial_body_focus_reveals_a_text_input_below_the_visible_body(cx: &mut TestAppContext) {
...
fn dialog_focus_target(
    ...
    let (invoker, body) = root.read_with(cx, |root, _| (root.invoker.clone(), root.body.clone()));
    ...
    fn present(&mut self, window: &Window, cx: &mut Context<Self>) {
        ...
        self.presentation = Some(
            Dialog::new(
                ...
                "Edit Options",
                vec![
                    ModalAction::new("save", "Save", ModalActionRole::Affirmative, "dialog-save")
                        .default_action(true),
                    ModalAction::new("cancel", "Cancel", ModalActionRole::Cancel, "dialog-cancel"),
                ],
                DialogInitialFocus::Action("save"),
            )
            .body(self.body.clone())
...
#[gpui::test]
fn command_period_closes_owned_menu_before_allowed_parent_cancellation(cx: &mut TestAppContext) {
    let (root, cx) = dialog_menu_window(cx);
