//! Shared RGBA color primitive and test-only fixtures for the extracted dark defaults.
//! Production appearance policy and resolution live in the Appearance Module.

mod color;
pub(crate) use color::Color;

#[cfg(test)]
pub(crate) use legacy::{ACTIVE_THEME, VAGUE_PRO};
...
mod legacy {
    ...
    impl Theme {
        ...
        pub(crate) fn from_zed_json(
            ...
            let family: Value =
                serde_json::from_str(json).map_err(|_| ThemeError::InvalidDocument)?;
            let themes = family
                .get("themes")
            ...
            if let Some(value) = style.get("accents") {
                ...
                if !values.is_empty() {
                    theme.accents = values
                        .iter()
                        .map(|value| {
                            optional_color(Some(value))
                                .map(|color| color.unwrap_or(theme.text_accent))
                        })
                        .collect::<Result<_, _>>()?;
                }
            }
