...
| **Native PTY Adapter Factory** | The narrow application-selected construction Interface that accepts one platform-neutral Native PTY launch and geometry and returns the Adapter and termination authority transferred into its Native PTY Owner. | platform singleton, broad Platform trait, PTY owner |
...
- Application composition selects the concrete **Native PTY Adapter Factory** and passes it through **Terminal Session** construction. The **Native PTY Owner** supplies only platform-neutral launch and geometry values and owns only the returned Adapter parts.
