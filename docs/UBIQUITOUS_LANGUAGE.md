...
- Portable SSH policy consumes the selected OpenSSH executable without exposing its path, applies the OpenSSH 8.2 minimum-version rule, validates every command specification, and clears ambient process environment before applying the captured allowlist and exact AskPass overlay.
- Portable application-path and managed-host policy owns XDG precedence, runtime-owner namespaces, artifact registration, mutation sequencing, conflicts, stale-owner refusal, and cleanup decisions. A **Secure Filesystem Adapter** exposes no raw descriptor, inode, ownership field, mode bit, or native error.
- Portable process supervision owns cancellation, deadlines, escalation, descendant-cleanup decisions, stale observation rejection, and exactly-once cleanup. An **SSH Process Adapter** exposes no process identifier, signal, process group, or raw native failure.
- The **AskPass Broker** validates a peer before parsing its bounded request frame, compares the capability in constant time, presents at most one generation-bound **Authentication Prompt**, zeroizes every secret response, and releases its exact endpoint once during teardown.
