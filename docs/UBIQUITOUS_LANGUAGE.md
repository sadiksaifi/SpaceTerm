...
| **File Insertion** | Ordered local absolute paths from validated clipboard file URLs or GPUI drag/drop, converted within input and aggregate bounds to POSIX-shell-quoted text before becoming a Paste Payload. | raw URL paste, direct PTY write |
| **Native Terminal Services** | The platform-neutral Module owning Selection publication, clipboard and File Insertion intake, Paste Payload and OSC 52 policy, contextual-action and Terminal Hyperlink revalidation, Quick Look eligibility and lifecycle, and exact Services request authority. | grab-bag platform Adapter, independent input path |
...
- The **Shell Launch Plan** owns executable and argument selection, exact working-directory validation, **Shell Integration**, inherited-environment handling, **Terminal Capability Identity**, **Compatibility Identity**, and resource decisions. Application composition injects only the host user shell and resource location.
