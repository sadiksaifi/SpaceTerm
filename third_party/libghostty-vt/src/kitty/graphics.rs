...
impl Terminal<'_, '_> {
    /// Copy renderer-ready geometry for all visible placements.
    ///
    /// Unicode placeholder cells are resolved by Ghostty on the emulator
    /// thread; no borrowed native pointers escape this call.
    pub fn resolved_kitty_placements(&self) -> Result<Vec<ResolvedPlacement>> {
        unsafe extern "C" fn collect(
            userdata: *mut std::ffi::c_void,
            placement: *const ffi::KittyGraphicsResolvedPlacement,
        ) -> bool {
            if userdata.is_null() || placement.is_null() {
                return false;
            }
            let output = unsafe { &mut *userdata.cast::<Vec<ResolvedPlacement>>() };
            let placement = unsafe { *placement };
            output.push(ResolvedPlacement {
        ...
        let mut placements: Vec<ResolvedPlacement> = Vec::new();
        let result = unsafe {
            ffi::ghostty_terminal_kitty_graphics_resolved_placements(
                self.inner.as_raw(),
                std::ptr::from_mut(&mut placements).cast(),
                Some(collect),
            )
        };
        from_result(result)?;
        placements.sort_unstable_by_key(|placement| (placement.z, placement.image_id));
        Ok(placements)
    }
...
#[cfg(all(feature = "kitty-graphics", feature = "png"))]
impl Default for RustPngDecoder {
    fn default() -> Self {
        Self::new()
    }
}
