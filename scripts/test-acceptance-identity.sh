...
test_native_observation_is_hashed_bound_and_font_consistent() {
    ...
    write_native_observation_fixture "$observation" "$app_sha256"
    write_record "$manifest" "native.observation.source" "production-app"
    write_record "$manifest" "native.observation.path" "identity/native-observation.tsv"
    ...
    write_record "$manifest" "font.selected.source" "production-app-observation"
    write_record "$manifest" "font.selected.family" "JetBrainsMono Nerd Font"
    write_record "$manifest" "host.terminal_font_selected" "JetBrainsMono Nerd Font"
    for key in rows columns logical_width logical_height backing_pixel_width backing_pixel_height; do
